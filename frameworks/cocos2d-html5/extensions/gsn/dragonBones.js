    ...
    (function (objects) {
        ...
        var DataParser = (function () {
            ...
            DataParser.parseDragonBonesData = function (rawData) {
                ...
                var frameRate = Number(rawData.getElementsByTagName(utils.ConstValues.DRAGON_BONES)[0].getAttributeNode(utils.ConstValues.A_FRAME_RATE).value);
                var data = new SkeletonData();
                data.name = rawData.getElementsByTagName(utils.ConstValues.DRAGON_BONES)[0].getAttributeNode(utils.ConstValues.A_NAME).value;

                var armatureObjectList = rawData.getElementsByTagName(utils.ConstValues.ARMATURE);
                for (var index in armatureObjectList) {
                    if (!armatureObjectList.hasOwnProperty(index)) continue;
                    var armatureObject = armatureObjectList[index];
                    data.addArmatureData(DataParser.parseArmatureXML2Data(armatureObject, data, frameRate));
                }
            ...
            DataParser.copyAllProperties = function(obj,xml)
            {

            };
            DataParser.copyAllPropertiesAndChildToObject = function(xml)
            {
                var obj = {};
                ...
                if (xml.hasChildNodes()) {
                    for(var i = 0; i < xml.childNodes.length; i++) {
                        ...
                        if (typeof(obj[nodeName]) == "undefined") {
                            obj[nodeName] = DataParser.copyAllPropertiesAndChildToObject(item);
                        } else {
                            ...
                            obj[nodeName].push(DataParser.copyAllPropertiesAndChildToObject(item));
                        }
			...
			DataParser.parseArmatureXML2Data = function (armatureObject, data, frameRate) {
                var armatureData = new ArmatureData();
                armatureData.name = armatureObject.getAttributeNode(utils.ConstValues.A_NAME).value;

                var boneObjectList = armatureObject.getElementsByTagName(utils.ConstValues.BONE);
                for (var index in boneObjectList) {
                    if (!boneObjectList.hasOwnProperty(index)) continue;
                    var boneObject = boneObjectList[index]; // XML obj
                    var boneObj =

                    DataParser.copyAllPropertiesAndChildToObject(boneObject);
                    armatureData.addBoneData(DataParser.parseBoneData(boneObj));
                }

                var skinObjectList = armatureObject.getElementsByTagName(utils.ConstValues.SKIN);
                for (var index in skinObjectList) {
                    if (!skinObjectList.hasOwnProperty(index)) continue;
                    var skinObject = skinObjectList[index];
                    var skinObj =
                    DataParser.copyAllPropertiesAndChildToObject(skinObject);
                    armatureData.addSkinData(DataParser.parseSkinData(skinObj, data));
                }

                utils.DBDataUtil.transformArmatureData(armatureData);
                armatureData.sortBoneDataList();

                var animationObjectList = armatureObject.getElementsByTagName(utils.ConstValues.ANIMATION);
                ...
                for (var index in animationObjectList) {
                    ...
                    DataParser.copyAllPropertiesAndChildToObject(animationObject);
                    armatureData.addAnimationData(DataParser.parseAnimationData(animationObj, armatureData, frameRate));
                }
            ...
            DataParser.convertXMLAtrributeNode = function(obj,objXML,atrribute)
            {
                obj[atrribute] = objXML.getAttributeNode(atrribute).value;
            };

            DataParser.parseArmatureXMLData = function (armatureObject, data, frameRate) {
                var armatureData = new ArmatureData();
                armatureData.name = armatureObject.getAttributeNode(utils.ConstValues.A_NAME).value;
                ...
                for (var index in boneObjectList) {
                    ...
                    var boneObject = boneObjectList[index];
                    var boneObj = {};
                    boneObj[utils.ConstValues.A_NAME] = boneObject.getAttributeNode(utils.ConstValues.A_NAME).value;
                    var tranform = boneObject.getElementsByTagName(utils.ConstValues.TRANSFORM)[0];
                    boneObj[utils.ConstValues.TRANSFORM] = {};
                    DataParser.convertXMLAtrributeNode(boneObj[utils.ConstValues.TRANSFORM],tranform,utils.ConstValues.A_X);
                    DataParser.convertXMLAtrributeNode(boneObj[utils.ConstValues.TRANSFORM],tranform,utils.ConstValues.A_Y);
                    DataParser.convertXMLAtrributeNode(boneObj[utils.ConstValues.TRANSFORM],tranform,utils.ConstValues.A_SKEW_X);
                    DataParser.convertXMLAtrributeNode(boneObj[utils.ConstValues.TRANSFORM],tranform,utils.ConstValues.A_SKEW_Y);
                    DataParser.convertXMLAtrributeNode(boneObj[utils.ConstValues.TRANSFORM],tranform,utils.ConstValues.A_SCALE_X);
                    DataParser.convertXMLAtrributeNode(boneObj[utils.ConstValues.TRANSFORM],tranform,utils.ConstValues.A_SCALE_Y);
                    armatureData.addBoneData(DataParser.parseBoneData(boneObj));
                }
                ...
                for (var index in skinObjectList) {
                    ...
                    skinObj[utils.ConstValues.SLOT] = [];
                    skinObj[utils.ConstValues.A_NAME] = skinObject.getAttributeNode(utils.ConstValues.A_NAME).value;


                    var slotXMLlist =  skinObject.getElementsByTagName(utils.ConstValues.SLOT);
                    for(var index in slotXMLlist)
                    {
                        if (!slotXMLlist.hasOwnProperty(index)) continue;
                        var slotXML = slotXMLlist[index];
                        var slot = {};
                        DataParser.convertXMLAtrributeNode(slot,slotXML,"name");
                        DataParser.convertXMLAtrributeNode(slot,slotXML,"parent");



                        DataParser.convertXMLAtrributeNode(slot,slotXML,"z");

                        slot["display"] = {};
                        DataParser.convertXMLAtrributeNode(slot["display"],slotXML.getElementsByTagName(utils.ConstValues.DISPLAY)[0],"name");
                        DataParser.convertXMLAtrributeNode(slot["display"],slotXML.getElementsByTagName(utils.ConstValues.DISPLAY)[0],"type");

                        slot["display"]["transform"] = {};
                        var tranform = slotXML.getElementsByTagName(utils.ConstValues.DISPLAY)[0].getElementsByTagName(utils.ConstValues.TRANSFORM)[0];
                        DataParser.convertXMLAtrributeNode(slot["display"]["transform"],tranform,utils.ConstValues.A_X);
                        DataParser.convertXMLAtrributeNode(slot["display"]["transform"],tranform,utils.ConstValues.A_Y);
                        DataParser.convertXMLAtrributeNode(slot["display"]["transform"],tranform,utils.ConstValues.A_SKEW_X);
                        DataParser.convertXMLAtrributeNode(slot["display"]["transform"],tranform,utils.ConstValues.A_SKEW_Y);
                        DataParser.convertXMLAtrributeNode(slot["display"]["transform"],tranform,utils.ConstValues.A_SCALE_X);
                        DataParser.convertXMLAtrributeNode(slot["display"]["transform"],tranform,utils.ConstValues.A_SCALE_Y);

                        skinObj[utils.ConstValues.SLOT].push(slot);
                ...
                for (var index in animationObjectList) {
                    ...
                    var animationObject = animationObjectList[index];
                    var animationObj = {};
                    animationObj[utils.ConstValues.TIMELINE] = [];
                    DataParser.convertXMLAtrributeNode(animationObj,animationObject,"name");
                    DataParser.convertXMLAtrributeNode(animationObj,animationObject,"fadeInTime");
                    DataParser.convertXMLAtrributeNode(animationObj,animationObject,"duration");
                    DataParser.convertXMLAtrributeNode(animationObj,animationObject,"scale");
                    DataParser.convertXMLAtrributeNode(animationObj,animationObject,"loop");
                    DataParser.convertXMLAtrributeNode(animationObj,animationObject,"autoTween");
                    DataParser.convertXMLAtrributeNode(animationObj,animationObject,"tweenEasing");

                    var timelineList = animationObject.getElementsByTagName(utils.ConstValues.TIMELINE);
                    for(var index in timelineList) {
                        if (!timelineList.hasOwnProperty(index)) continue;
                        var timeLine = timelineList[index];
                        var timeLineObj = {};
                        DataParser.convertXMLAtrributeNode(timeLineObj,timeLine,"name");
                        DataParser.convertXMLAtrributeNode(timeLineObj,timeLine,"scale");
                        DataParser.convertXMLAtrributeNode(timeLineObj,timeLine,"offset");


                        timeLineObj[utils.ConstValues.FRAME] = [];
                        var frameList = timeLine.getElementsByTagName(utils.ConstValues.FRAME);
                        for(var idx in frameList)
                        {
                            if(!frameList.hasOwnProperty(idx)) continue;
                            var frameObj = {};
                            var frame = frameList[idx];
                            for(var i=0;i<frame.attributes.length;i++)
                            {
                                var attribute = frame.attributes.item(i);
                                frameObj[attribute.nodeName] = attribute.nodeValue;
                            }
                            timeLineObj[utils.ConstValues.FRAME].push(frameObj);
                        }

                        animationObj[utils.ConstValues.TIMELINE].push(timeLineObj);
                    }
