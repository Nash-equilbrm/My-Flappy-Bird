...
(function (db) {
    var DBCCFactory = (function (_super) {
        ...
        DBCCFactory.prototype._generateArmature = function () {
            var armatureNode = db.DBCCArmatureNode.create();
            var armature = new dragonBones.Armature(armatureNode);
        ...
        DBCCFactory.prototype._generateDisplay = function (textureAtlas, fullName, pivotX, pivotY) {
            ...
            var anchorX = (pivotX / sprite.getContentSize().width);// * -1;
            var anchorY = 1 - (pivotY / sprite.getContentSize().height);// * -1;
        ...
        DBCCFactory.prototype.buildArmatureNode = function (armatureName, dragonBonesName, skinName) {
            var armature = this.buildArmature(armatureName, null, dragonBonesName,null, skinName);
            var armatureNode = armature ? armature._display : null;
            if (armatureNode)
            {
